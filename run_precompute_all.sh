...
echo -e "\n[Step 1] Syncing code on all workers..."
...
pkill -9 -f precompute_embeddings 2>/dev/null || true
for WORKER_ID in $(seq 1 $((NUM_WORKERS - 1))); do
    gcloud compute tpus tpu-vm ssh "$INSTANCE" \
        --zone="$ZONE" \
        --worker="$WORKER_ID" \
        --command="pkill -9 -f precompute_embeddings 2>/dev/null || true" &
done
wait
echo "  Cleaned"
...
cd ~/ouroboros
nohup bash -c "TPU_WORKER_ID=0 python3 -u precompute_embeddings_distributed.py" \
    > /tmp/precompute_worker_0.log 2>&1 &
echo "  Worker 0 started (PID: $!)"
...
for WORKER_ID in $(seq 1 $((NUM_WORKERS - 1))); do
    ...
    gcloud compute tpus tpu-vm ssh "$INSTANCE" \
        --zone="$ZONE" \
        --worker="$WORKER_ID" \
        --command="cd ~/ouroboros && nohup bash -c 'TPU_WORKER_ID=$WORKER_ID python3 -u precompute_embeddings_distributed.py' > /tmp/precompute_worker_$WORKER_ID.log 2>&1 &" &
done
wait
