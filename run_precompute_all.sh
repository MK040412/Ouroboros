...
echo "  Worker N: gcloud compute tpus tpu-vm ssh $INSTANCE --zone=$ZONE --worker=N --command='tail -f /tmp/precompute_worker_N.log'"
echo "  (Replace N with worker ID: 1-7)"
echo ""
echo "Expected time: ~2.5-4 hours (92 files / 8 workers × ~15 min/file)"
