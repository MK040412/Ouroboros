...
# =============================================================================
# TPU v5e-32 (vtlitepod-32) ImageNet 분산 학습 스크립트
# =============================================================================
# TPU v5e-32 구성:
...
#
# 사용법:
#   로컬에서 실행: ./run_imagenet_distributed.sh           # 자동 resume
#   새로 시작:     ./run_imagenet_distributed.sh --fresh   # 새 학습 시작
#   RAM 프리로드 비활성화: ./run_imagenet_distributed.sh --no-ram-preload
# =============================================================================
...
while [[ $# -gt 0 ]]; do
  case $1 in
    --fresh)
      ...
      ;;
    --no-ram-preload)
      NO_RAM_PRELOAD="--no-ram-preload"
      shift
      ;;
    *)
      echo "Unknown option: $1"
      echo "Usage: $0 [--fresh] [--no-ram-preload]"
      exit 1
      ;;
  esac
done
...
# =============================================================================
run_remote() {
  log "Running ImageNet training in REMOTE mode"
...
EOF
  ...
  # 5. 모든 worker에서 ImageNet 학습 시작 (--worker=all 사용)
  log "Step 5: Starting ImageNet training on all workers..."
...
cd ~/ouroboros
echo "[Worker \$WORKER_ID] Starting ImageNet training..."
...
# nohup으로 백그라운드 실행
nohup python3.11 -u train_imagenet_tpu.py $FRESH_START $NO_RAM_PRELOAD > /tmp/imagenet_train_worker_\${WORKER_ID}.log 2>&1 &
...
EOF
  ...
  log ""
  log "Dataset: ImageNet-1K (gs://rdy-tpu-data-2025/imagenet-1k/)"
  log "Script: train_imagenet_tpu.py"
  if [[ -n "$FRESH_START" ]]; then
    log "Mode: FRESH START"
  else
    log "Mode: AUTO RESUME"
  fi
  if [[ -n "$NO_RAM_PRELOAD" ]]; then
    log "RAM Preload: DISABLED (streaming from GCS)"
  else
    log "RAM Preload: ENABLED"
  fi
  log ""
  log "Monitor logs:"
  log "  ./gcp_run.sh 'tail -50 /tmp/imagenet_train_worker_0.log'"
  log "  ./gcp_run.sh 'tail -f /tmp/imagenet_train_worker_0.log'"
  log ""
